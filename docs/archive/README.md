...
1. Modify the shebang line of the `chatgpt-cli` file to use the path of your Python
...
1. Finally, set the execute permission for the `chatgpt-cli` file if it does not have
   currently:
...
1. Add the following line to your shell run command file, such as `.bashrc`, to include
...
1. Type `source ~/.bashrc` (or a similar command) to start using this tool from any
   directory.
...
1. Open the environment variable settings window by searching for "Environment Variables"
   in the Windows search bar.

1. Click on "Edit the system environment variables" and then click on the "Environment
   Variables" button.

1. In the "User variables" or "System variables" section, find the PATH variable, and
   click on the "Edit" button.

1. In the "Edit environment variable" window, click on the "New" button and add the
   absolute path to the bin directory (e.g., `C:\Projects\chatgpt-cli\bin`). Click on "OK"
   to close all the windows.

1. In the bin directory, create a new file named `chatgpt-cli.bat` and paste the following
   contents:
