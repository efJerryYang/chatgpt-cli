...
1. Then, copy the `chat.py` file to the `bin` directory and rename it to `chatgpt-cli`:

   ```sh
   cp -a chat.py bin/chatgpt-cli
   ```

1. Modify the shebang line of the `chatgpt-cli` file to use the path of your Python interpreter that installed the `requirements.txt`.
...
1. Finally, set the execute permission for the `chatgpt-cli` file if it does not have currently:

   ```sh
   sudo chmod a+x bin/chatgpt-cli
   ```

1. Add the following line to your shell run command file, such as `.bashrc`, to include the `bin` directory to your `$PATH` environment variable:
...
1. Type `source ~/.bashrc` (or a similar command) to start using this tool from any directory.
...
1. Copy the `chat.py` file to the bin directory and rename it to `chatgpt-cli.py`.

1. Open the environment variable settings window by searching for "Environment Variables" in the Windows search bar.

1. Click on "Edit the system environment variables" and then click on the "Environment Variables" button.

1. In the "User variables" or "System variables" section, find the PATH variable, and click on the "Edit" button.

1. In the "Edit environment variable" window, click on the "New" button and add the absolute path to the bin directory (e.g., `C:\Projects\chatgpt-cli\bin`). Click on "OK" to close all the windows.

1. In the bin directory, create a new file named `chatgpt-cli.bat` and paste the following contents:
...
1. Open a new command prompt or PowerShell window and run the `chatgpt-cli` command.
